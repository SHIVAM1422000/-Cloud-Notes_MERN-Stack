...
router.post(
  ...
  async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      return res.status(400).json({ success, error: "Wrong Credentials",body:req.body});
    }
    ...
    try {
      ...
      return res.status(500).json({ success, error: "Internal Server Error" , msg: error.message});
    }
  }
);
